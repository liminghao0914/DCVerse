...
class ReticleOverlay:
  ...
  def __init__(self, model: ReticleModel, vp_win: ui.Window, ext_id: str):
    ...
    try:
      ...
      self._view_change_sub = self.vp_win.viewport_api.subscribe_to_view_change(
          self.on_window_changed)
    except AttributeError:
      carb.log_info(
          "Using Viewport Legacy: Reticle will not automatically update on resolution changes.")
        ...
        with ui.ZStack():
          ...
          if self.vp_win.width / self.vp_win.height > self.get_aspect_ratio_flip_threshold():
            self.scene_view = scene.SceneView(
                aspect_ratio_policy=scene.AspectRatioPolicy.PRESERVE_ASPECT_VERTICAL)
          else:
            self.scene_view = scene.SceneView(
                aspect_ratio_policy=scene.AspectRatioPolicy.PRESERVE_ASPECT_HORIZONTAL)
  ...
  def _build_safe_rect(self, percentage, color):
    ...
    inverse_ratio = 1 / aspect_ratio
    if self.scene_view.aspect_ratio_policy == scene.AspectRatioPolicy.PRESERVE_ASPECT_VERTICAL:
      scene.Rectangle(aspect_ratio*2*percentage, 1*2*percentage,
                      thickness=1, wireframe=True, color=color)
    else:
      scene.Rectangle(1*2*percentage, inverse_ratio*2*percentage,
                      thickness=1, wireframe=True, color=color)
...
class ReticleMenu:
  ...
  def __init__(self, model: ReticleModel):
    ...
    """
    self.model = model
    self.button = ui.Button("DCReticle", width=0, height=0, mouse_pressed_fn=self.show_reticle_menu,
